...
<div class="max-w-3xl mx-auto py-8">
    <h1 class="text-3xl font-bold mb-6 text-center text-gray-800">Editar Tarea</h1>

    <form action="{{ route('tasks.update', $task) }}" method="POST" class="bg-white shadow-md rounded-2xl p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="title" id="title" 
                ...
                placeholder="Título de la tarea" value="{{ old('title', $task->title) }}">
            @error('title')
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-700 transition"
                placeholder="Descripción de la tarea">{{ old('description', $task->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="event_id" id="event_id"
                ...
                @forelse ($events as $event)
                    <option value="{{ $event->id }}" @selected(old('event_id', $task->event_id) == $event->id)>
        ...
        <div class="flex justify-end gap-2 pt-4">
            <a href="{{ url()->previous() }}"
                class="flex items-center gap-1 text-gray-700 hover:text-gray-600 transition px-4 py-2 rounded shadow bg-gray-100">
